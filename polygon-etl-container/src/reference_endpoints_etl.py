...
def start(endpoint, api_key, s3_client):
    ...
    logger.info('Starting reference endpoint collection:%s', endpoint)
    ...
    out_key_timestamped = str(pathlib.PurePosixPath(data_folder, 'reference', endpoint, 'timestamped',
                                                    f'{endpoint}--{hf.get_file_timestamp()}.csv'))

    url = f'https://api.polygon.io/v3/reference/{endpoint}?apiKey={api_key}&limit=1000'
    all_results = []
    while True:
        debug_url = url.replace(api_key, 'xXx_API_KEY_xXx')
        logger.debug('Current cumulative record count:%s. Calling endpoint %s', len(all_results), debug_url)
        response = requests.get(url)

        if response.status_code != 200:
            logger.debug('reference/%s endpoint returned non-200 status_code.', endpoint)
            try:
                logger.debug(f'Status code:%s', response.status_code)
